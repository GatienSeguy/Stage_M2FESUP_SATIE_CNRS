...
from emg_vba.emg_vba_1_dim_torch import EMG_VBA


# =====================================================================
# Pas reverse DDPM et VE
# =====================================================================
...
# =====================================================================
# Correction EMG-VBA à un pas t donné
# =====================================================================
...
# =====================================================================
# Sampling conditionnel complet
# =====================================================================

@torch.no_grad()
def sample_conditional(net, schedule, y_flat, op, shape,
                       n_samples=1, device='cuda',
                       emg_n_iter=100, emg_skip_after=0,
                       a_0=1e-3, b_0=1e-3, c_0=1e-3, d_0=1e-3,
    ...
    for t_val in tqdm(reversed(range(schedule.T)), total=schedule.T, desc="Sampling conditionnel"):
        ...
        if t_val >= emg_skip_after:
            ...
            for b in range(n_samples):
                ...
                # Diagnostics (premier sample uniquement)
                if b == 0:
                    if t_val in monitor_set:
        ...
        if t_val in snapshot_steps:
            ...
            if t_val >= emg_skip_after:
                diagnostics['sigma_per_step'][t_val] = state['Sigma'].cpu().numpy()
