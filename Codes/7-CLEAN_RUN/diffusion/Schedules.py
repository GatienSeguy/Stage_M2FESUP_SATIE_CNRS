...
class DDPMSchedule(NoiseSchedule):
    ...
    @classmethod
    def from_alphas_cumprod(cls, alphas_cumprod):
        """Construit le schedule depuis les alphas_cumprod (ex: diffusers)."""
        obj = cls.__new__(cls)
        obj.variant = 'ddpm'
        obj.alphas_bar = alphas_cumprod
        obj.T = len(alphas_cumprod)
        obj.alphas = torch.cat([alphas_cumprod[:1], alphas_cumprod[1:] / alphas_cumprod[:-1]])
        obj.betas = 1.0 - obj.alphas
        obj.sigmas = torch.sqrt(1.0 - alphas_cumprod)
        return obj
