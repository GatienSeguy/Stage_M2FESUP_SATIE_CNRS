...
from operateur.operators_torch import GaussianBlurOperator
...
def make_plots(results, diag, output_dir, run_name,
    ...
    # 7bis. sigma_r et sigma_b par pas monitored (même format que énergie libre)
    energie = diag.get('energie_par_step') or {}
    if energie:
        ts_e = sorted(energie.keys(), reverse=True)
        n_e = len(ts_e)
        ...
        for idx, t_val in enumerate(ts_e):
            ax = axes[idx // ncols][idx % ncols]
            data = energie[t_val]
            iters = range(1, len(data['tau_r']) + 1)
            sig2_r = [1.0 / tr for tr in data['tau_r']]
            sig2_b = [1.0 / tb for tb in data['tau_b']]
            ax.plot(iters, sig2_r, 'r-o', markersize=2, label=r'$\sigma_r^2$')
            ax.plot(iters, sig2_b, 'b-s', markersize=2, label=r'$\sigma_b^2$')
            ax.set_title(f"t = {t_val}")
            ax.set_xlabel("Itération k")
            ax.set_yscale('log')
            ax.grid(True, alpha=0.3)
            if idx == 0:
                ax.legend(fontsize=7)
        fig.suptitle(r"$\sigma_r^2$ et $\sigma_b^2$ par itération EMG-VBA à t fixé")
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f"{run_name}_emgvba_sigmas_per_step_{suffix}.png"), dpi=150)
        plt.close(fig)
