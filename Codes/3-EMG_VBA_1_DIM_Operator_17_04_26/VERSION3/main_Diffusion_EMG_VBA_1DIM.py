...
OUTPUT_DIR = os.path.join(BASE_DIR, 'resultats/TEST')
...
OPERATOR = GaussianBlurOperator(kernel_size=9, sigma=SIGMA_BLUR, img_size=IMG_SIZE)
# ==============


# def degrade_image(image_path, op, img_size, output_dir):
#     """Crée l'observation y = A x₀  (sans bruit additif ici)."""
...
#     # Observation via l'opérateur
#     y_11 = op.create_observation(x0_11,SIGMA_NOISE).reshape(img_size, img_size)
...
#     print(f"[Dégradation] {img_size}x{img_size}, opérateur : {type(op).__name__}")
...
def degrade_image(image_path, op, img_size, n_channels, output_dir):
    ...
    # Charger en RGB ou grayscale selon n_channels
    ...
    # Pour RGB : (H, W, 3) → (3, H, W)
    if n_channels == 3 and x0.ndim == 3:
        x0 = x0.transpose(2, 0, 1)

    # Passage en [-1, 1]
    x0_11 = 2.0 * x0 - 1.0

    # Observation y = Ax0 + sigma_b * epsilon
    y_11 = op.create_observation(x0_11.ravel(), SIGMA_NOISE).reshape(x0_11.shape)
...
# def load_net(ckpt_dir, device):
#     ckpt_path = os.path.join(ckpt_dir, "ddpm_ema_celebahq_256.pt")
#     ckpt = torch.load(ckpt_path, map_location=device, weights_only=False)

#     from diffusers import UNet2DModel
#     net = UNet2DModel(**ckpt['config']).to(device)
#     net.load_state_dict(ckpt['model_state_dict'])
#     net.eval()

#     # Construire le schedule à partir des alphas_cumprod sauvegardés
#     alphas_cumprod = ckpt['alphas_cumprod']

#     print(f"[Modèle] Chargé : {ckpt_path}")
#     return net, alphas_cumprod
...
def main():
    ...
    # 1. Dégradation
    name, x0_11, y_11 = degrade_image(IMAGE_DIR, op, IMG_SIZE, N_CHANNELS, OUTPUT_DIR)
    ...
    shape = (N_CHANNELS, IMG_SIZE, IMG_SIZE)
