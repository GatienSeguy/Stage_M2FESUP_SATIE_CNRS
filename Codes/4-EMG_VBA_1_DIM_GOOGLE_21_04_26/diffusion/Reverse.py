...
def emg_vba_correction(x_t_np, xhat0_np, alpha_bar_t, y_np, op,
                       n_iter=5,
...
@torch.no_grad()
def sample_conditional(net, schedule, y_np, op, shape,
        ...
        if t_val >= emg_skip_after:
            ...
            for b in range(n_samples):
                ...
                mu_post, state = emg_vba_correction(
                    x_t_np, xhat0_np, alpha_bar_t, y_np, op,
                    n_iter=emg_n_iter,
                    a_0=a_0, b_0=b_0, c_0=c_0, d_0=d_0,
                    Aty=Aty, AtA_diag=AtA_diag,
                ...
                if b == 0 and t_val in monitor_set:
                    diagnostics['energie_par_step'][t_val] = {
                        'F': state['historique']['energie_libre'],
                        'log_jointe': state['historique']['log_jointe'],
                        'entropie': state['historique']['entropie'],
                        'forward': state['historique']['forward'],
                        'mesure': state['historique']['mesure'],
                        'a_priori': state['historique']['a_priori'],
                    }
