...
class EMG_VBA:
    def __init__(self, op, y, alpha_bar_t, xt, xhat0, a_0, b_0, c_0, d_0,
                 Aty=None, AtA_diag=None, device=None, dtype=torch.float64):
        ...
        self.AtA_diag = self._to_tensor(AtA_diag)   # diag(A^T A), précalculé à l'extérieur
    ...
    def mise_a_jour_taur_taub(self):
        ...
        self.b_tilde_b = (self.d_0 + 0.5 * (torch.dot(res, res) + torch.dot(self.AtA_diag, self.Sigma))) # (76)
        ...
        self.b_tilde_r = (self.b_0 + 0.5 * (torch.dot(diff, diff) + torch.sum(self.Sigma))) #(85)
    ...
    def calculer_pas_sousopt(self):
        ...
        terme_M = (coeff * torch.dot(d1_mu, d1_mu) + self.tau_b_moy * torch.dot(A_d1_mu, A_d1_mu))

        # Terme D
        terme_D = 0.5 * torch.sum(d1_Sigma**2 / self.Sigma**2)

        # Hessienne totale
        H11 = (torch.dot(G_mu, d11_mu) + torch.dot(G_Sigma, d11_Sigma) - terme_M - terme_D)
    ...
    def calculer_energie_libre_negative(self):
        ...
        Forward = -0.5 / (1 - abar) * (torch.dot(forward_diff, forward_diff) + abar * torch.sum(self.Sigma))

        ## Mesure - (123)
        mesure_diff = self.y - self._A_mu

        Mesure = ((self.m / 2.0 + self.c_0 - 1.0) * log_tau_b - (self.d_0 + 0.5 * (torch.dot(mesure_diff, mesure_diff) + torch.dot(self.AtA_diag, self.Sigma))) * tau_b)


        #A priori
        a_priori_diff = self.mu - self.xhat0

        A_priori = ((self.n / 2.0 + self.a_0 - 1.0) * log_tau_r - (self.b_0 + 0.5 * (torch.dot(a_priori_diff, a_priori_diff) + torch.sum(self.Sigma))) * tau_r)
    ...
    def executer(self, n_iter, mu_init, Sigma_init, a_0_init, b_0_init, c_0_init, d_0_init, verbose=True, affichage=True):
        ...
        for k in range(n_iter):
            ...
            E_k = Energie_libre_negative.detach().cpu().item()

            historique['energie_libre'].append(E_k)
            historique['s1'].append(s1.detach().cpu().item())
            historique['tau_r'].append(self.tau_r_moy.detach().cpu().item())
            historique['tau_b'].append(self.tau_b_moy.detach().cpu().item())

            if verbose and (k % 10 == 0 or k == n_iter - 1):
                print(f"  k={k:3d} | NRJ libre negative={historique['energie_libre'][-1]:+.2f} | s1={historique['s1'][-1]:.4f} | "
                      f"r2_t={1/historique['tau_r'][-1]:.6f} | "
                      f"sig2_b={1/historique['tau_b'][-1]:.6f}")

            # Critère d'arrêt : variation relative de l'énergie libre
